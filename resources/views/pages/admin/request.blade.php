            ...
            <div class="card card-post card-round" style="border-top: 3px solid #f25961;">
                ...
                @if($userInfo->usertype === 2)
                   <form id="acceptForm" style="margin-bottom: 0px;"
                        ...
                        method="POST">
                        @csrf
                        <button class="btn btn-danger w-100 bubble-shadow" id="alert_demo_8">Accept</button>
                    </form> 
                @endif
...
<div class="row">
    ...
    <!-- PUT A FOR-LOOP CONTITION HERE -->
    @forelse($job_tranferred as $transferred)
        @foreach($transferred->transferedRequests as $trans)
            @if((int)$trans->transfer_from === (int)$userInfo->userid || $userInfo->usertype == 1)
                <div class="col-md-3">
                    <div class="card card-post card-round" style="border-top: 3px solid #ffad46;">
                        <div class="card-body">
                            <div class="d-flex">
                                ...
                                <div class="info-post ms-2">
                                    <p class="username"> {{$transferred->requester->fname . ' ' . $transferred->requester->lname}}</p>
                                    <p class="date text-muted">{{$transferred->requester->sectionRel->acronym}} Section {{ $transferred->requester->divisionRel->description}}</p>
                                </div>
                            </div>
                            <div class="separator-solid"></div>
                            <p class="card-category text-info mb-1">
                                <a>{{\Carbon\Carbon::parse($transferred->request_date)->format('F d, Y h:i A')}}</a>
                            </p>
                            <h3 class="card-title">
                            ...
                            <div>
                                ...
                                <ul>
                                @php
                                    $tasks = explode(',', $transferred->description);
                                    $tasks = array_map('trim', $tasks); // Trim whitespace from each item
                                @endphp
                                @foreach($tasks as $index => $task)
                                    @if($task === 'Others' && isset($tasks[$index + 1]))
                        ...
                        @php
                            $dtrUser = Dtruser::where('userid',$trans->transfer_from)->first();
                            $dtrUser_to = Dtruser::where('userid',$trans->transfer_to)->first();
                        @endphp
                        @if($userInfo->usertype == 1)
